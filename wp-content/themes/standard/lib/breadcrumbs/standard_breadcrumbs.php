	...
	public static function get_breadcrumb_trail( $page_id ) { 
		...
		if( ! is_home() ) {
			...
			if( is_page() ) {
				...
				if( is_author() ) {
					$str_breadcrumb .= self::get_home_link() . __( 'posts', 'standard' ) . __( ' / ', 'standard' ) . self::get_author_display_name(); 
				} elseif( '' != get_query_var( 'year' ) || '' != get_query_var( 'monthnum' ) || '' != get_query_var( 'm' ) || '' != get_query_var( 'day' ) ) {
					$str_breadcrumb .= self::get_home_link() . __( 'posts', 'standard' ) . __( ' / ', 'standard' ) . self::get_date_labels(); 
				} else {
					$str_breadcrumb .= self::get_home_link() . __( 'posts', 'standard' ) . __( ' / ', 'standard' ) . self::get_category_links();
				} // end if
